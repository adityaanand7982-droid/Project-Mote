/**
 * ============================================================================
 *  Project-Mote : High-Altitude Airborne Microclimate & Pollution Tracker
 * ============================================================================
 *  Target Board : Seeed Studio XIAO ESP32-S3 Sense
 *  Sensors      : 7Semi BME688 Nano Breakout (I2C, addr 0x77)
 *                 SparkFun BMV080 Qwiic (I2C, addr 0x57)
 *  Storage      : Onboard MicroSD (SPI, CS=3)
 *  Camera       : Onboard OV2640
 *  Telemetry    : HardwareSerial (GPS from drone flight controller)
 *
 *  Architecture : Non-blocking cooperative scheduler using millis()
 *  Data Format  : CSV row appended per measurement cycle
 *  Photo Format : JPEG named by GPS UTC timestamp (HHMMSS.jpg)
 * ============================================================================
 */
...
#include <7Semi_BME68x.h>               // 7Semi BME688 library
#include <SparkFun_BMV080_Arduino_Library.h> // SparkFun BMV080 library
#include <ESP32Time.h>
...
#define GPS_RX_PIN            44    // ESP32 RX  <- Flight Controller TX
#define GPS_TX_PIN            43    // ESP32 TX  -> Flight Controller RX (optional)
#define GPS_BAUD              115200 
#define BMV080_ADDR           0x57  // SparkFun BMV080 default I2C address

TinyGPSPlus gps;
HardwareSerial gpsSerial(1);   // UART1

ESP32Time rtc(0);              // UTC offset = 0 (GPS provides UTC)
BME68x_7Semi bme;

SparkFunBMV080 pmSensor;

File logFile;
char csvFilename[32];          // e.g. "/logs/flight_log_01.csv"
...
const unsigned long MEASUREMENT_INTERVAL_MS = 3000UL;   // 3 seconds
...
void setup() {
  Serial.begin(115200);
  // NOTE: The blocking `while(!Serial)` line has been removed.
  // This allows the device to boot autonomously from battery power.
  delay(500); // Brief pause to allow USB serial to settle (if connected).
...
void loop() {
  // 1. Continuously feed GPS parser (never block on this).
  while (gpsSerial.available() > 0) {
    gps.encode(gpsSerial.read());
  }

  // 2. Check time managmnt 
  ...
  // 3. Sync RTC whenever a fresh GPS time fix arrives
...
void initI2C() {
  ...
  Wire.begin();
  Wire.setClock(400000);   // Fast mode for Bosch sensors
...
void initCamera() {
  ...
  if (psramFound()) {
    ...
    // Fallback: low resolution without PSRAM.
...
bool readBMV080(MoteData &d) {
  // readSensor() returns true only when a fresh measurement is available.
